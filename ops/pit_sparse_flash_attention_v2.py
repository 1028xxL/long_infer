...
def _triton_mixed_sparse_attn_fwd_kernel(
    ...
    # for start_n in range(0, num_cols, 128):
    for start_n in range(0, num_cols, BLOCK_N):
        ...
        v = tl.load(v_ptrs + cols[:, None] * stride_vn, mask=n_mask[:, None], other=0.0)
        # -- compute qk --
        # qk = tl.zeros([BLOCK_M, 128], dtype=tl.float32)
        qk = tl.zeros([BLOCK_M, BLOCK_N], dtype=tl.float32)
...
def _triton_mixed_sparse_attention(
    ...
    # print(f"q.strde: {q.stride()}")
    # print(f"q.stride(0): {q.stride(0)}; q.stride(1): {q.stride(1)}; q.stride(2): {q.stride(2)}; q.stride(3): {q.stride(3)};")
    # print(f"block_count.shape: {block_count.shape}")
    # print(f"block_offset.shape: {block_offset.shape}")
    # print(f"column_count.shape: {column_count.shape}")
    # print(f"column_index.shape: {column_index.shape}")
    # print(f"block_size_M: {block_size_M}")
    # print(f"block_size_N: {block_size_N}")
    # print(f"block_dmodel: {Lk}")
    # print(f"dtype: {dtype}")
    # print(f"seqlens: {seqlens}") # tensor([10011], device='cuda:0', dtype=torch.int32)  10000->10011
    # exit()
...
def vertical_slash_sparse_attention(
    ...
    # print(f"block_offset[0][0][0]: {torch.count_nonzero(block_offset[0][0][0])}\n{block_offset[0][0][0][:10]}")
    # print(f"block_offset[0][0][155]: {torch.count_nonzero(block_offset[0][0][155])}\n{block_offset[0][0][155][:10]}")
    # print(f"block_offset[0][0][156]: {torch.count_nonzero(block_offset[0][0][156])}\n{block_offset[0][0][156][:10]}")
    # print(f"block_count_shape: {block_count.shape}; \nblock_offset_shape: {block_offset.shape}; \ncolumn_count_shape: {column_count.shape}; \ncolumn_index_shape: {column_index.shape}\n")
    ...
    global total_num
    total_num += 1
    # total_sparsity += torch.sum(block_count) / (triton.cdiv(query.shape[2], block_size_M) ** 2) * 2 
    total_sparsity += (torch.sum(block_count) + torch.sum(column_count) / block_size_N) / (triton.cdiv(query.shape[2], block_size_M) ** 2) * 2 
    if(total_num % 200 == 0):
        print(f"num: {total_num}; sparsity: {total_sparsity / total_num}")
        # print(f"num: {total_num}; colNum: {colNum}")
        # total_num = 0
        # total_sparsity = 0
    ...
    # print(out[..., :context_size, :head_dim])
    # exit()
