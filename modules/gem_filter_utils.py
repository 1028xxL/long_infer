...
def standard_dis_index(data, queries, k, norm=1, pool=False, kernel_size=5, sum_over_heads=False):
    ...
    inner_product = inner_product[:, :, 0, :] # 为了确保只有一个query的结果 探索选择的query数量，需要进一步求和
    if sum_over_heads:
...
def find_context(self, query_states, key_states, topk, print_idx_dis=False):
    ...
    if self.indecies is None and self.layer_idx == self.select_layer_idx:
        ...
        query_last_states = query_states[:, :, -1:, :] # 选择最后一个roken的query
        _, indices = standard_dis_index(key_states_repeat, query_last_states, min(topk, n), pool=True, sum_over_heads=True)
