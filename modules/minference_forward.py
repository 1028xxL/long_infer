...
from ..ops.pit_my_sparse import _triton_mixed_sparse_attention

# from flash_attn import flash_attn_func
# from flash_attn.flash_attn_triton import flash_attn_func # flashAttention自带 似乎有bug
# 
from ..ops.flash_attn_triton import _flash_attn_triton_decoding as flash_attn_func # 使用triton方法
from ..ops.flash_attn_triton_my import _flash_attn_triton_decoding as flash_attn_func1 # 使用my triton方法
from ..ops.flash_attn_triton_my_v2 import _flash_attn_triton_decoding as flash_attn_func2 # 使用my triton方法 V2
from ..ops.flash_attn_triton_my_v3 import _flash_attn_triton_decoding as flash_attn_func3 # 使用my triton方法 V3
from ..ops.flash_attn_triton_my_v4 import _flash_attn_triton_decoding as flash_attn_func4 # 使用my triton方法 V4
from ..ops.flash_attn_triton_my_v5 import _flash_attn_triton_decoding as flash_attn_func5 # 使用my triton方法 V5
...
def gather_last_q_vertical_slash_topk_v4(self, q, k, v, head_id, layer_idx=0):
    kv_seq_len = k.size(2)
    ...
    def vertical_and_slash_kernel(q, k, v, vertical_size, slash_size):
        ...
        # vit = True
        vit = False
        if(head_id == 31 and layer_idx == 0 and vit): 
            scores = torch.matmul(q, k.transpose(-2, -1)) / (q.size(-1) ** 0.5)
            # mask shape: [1, 1, seq_len, seq_len]
            seq_len = q.size(-2)
            mask = torch.tril(torch.ones((seq_len, seq_len), device=q.device, dtype=torch.bool)).unsqueeze(0).unsqueeze(0)
            scores = scores.masked_fill(~mask, float('-inf'))
            attn = torch.nn.functional.softmax(scores, dim=-1)
            sparsity_mask = attn[0][0] > 0.005
            print(torch.sum(sparsity_mask) / seq_len / seq_len)
            ...
            import pandas as pd
            sparsity_mask_cpu = sparsity_mask.cpu().numpy()
            sparsity_sparse = sp.coo_matrix(sparsity_mask_cpu)
            true_coords = sparsity_sparse.nonzero()
            x = true_coords[1]  # 列索引
            y = seq_len - 1 - true_coords[0]  # 行索引
            df = pd.DataFrame({'x': x, 'y': y})
            canvas = ds.Canvas(plot_width=2000, plot_height=2000)
            agg = canvas.points(df, 'x', 'y')
            img = tf.shade(agg, cmap=viridis[::10], how='eq_hist')
            # img = tf.shade(agg, cmap=["white", "blue"], how='linear', span=(0, 1))
            img_pil = img.to_pil()
            img_pil.save("sparsity_mask.png")
            exit()


        # if(q_len < 1000):  # 小于1000使用密集注意力
        ...
        # print(q)
        # return flash_attn_func3(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1) # my v3
        # return flash_attn_func4(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1) # my v4
        # return flash_attn_func5(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1) # my v5


        # Minfer
        vertical_size, slash_size  = min(q_len, max(vertical_size, 30)), min(q_len, max(slash_size, 50))

        # random_permutation1 = (torch.randperm(8000) + 10)[:1000]
        # vertical_topk = random_permutation1.view(1, 1, 1, -1)
        # random_permutation2 = (torch.randperm(8000) + 10)[:6096]
        # slash = random_permutation2.view(1, 1, -1)
        ...
        # print(qk.shape) # [1, 1, 64, 10011]; L = 10000
        # print(vertical.shape) # [1, 1, 1, 10011]
        # exit()

        # my N:M
        # b, h, _, l = vertical.shape
        # m = 64 
        # n = 16
        # padding_size = m - (l % m)
        # vertical = F.pad(vertical, (0, padding_size), value=float('-inf'))
        # l = vertical.size(-1)  # 更新 l 的值，确保是填充后的长度
        # vertical_grouped = vertical.view(b, h, 1, l // m, m)
        # vertical_N_M = torch.topk(vertical_grouped, n, dim=-1).indices
        # group_start_idx = torch.arange(0, l, m, device=vertical.device).view(b, h, 1, -1, 1)  # 每组的起始位置
        # vertical_N_M = vertical_N_M + group_start_idx  # 加上偏移量，得到全局索引
        # vertical_N_M = vertical_N_M.view(b, h, 1, -1)

        vertical[...,:30] = torch.inf
        vertical_topk = torch.topk(vertical, vertical_size, -1).indices

        # print(vertical_N_M.shape)
        # print(vertical_N_M)
        ...
        # s = time.time() - start
        # print(f"长度：{q_len}, 预测时间: {s}")
        # exit()

        # print(f"vertical_topk: {vertical_topk}\n slash: {slash}\n")
        # print(f"vertical_topk.shape: {vertical_topk.shape}\n slash.shape: {slash.shape}\n")
        # exit()

        # print(q)

        return vertical_slash_sparse_attention(q, k, v, vertical_topk, slash)
        # return vertical_slash_sparse_attention(q, k, v, vertical_N_M, slash)


        # _, _, seqlens, head_dim = q.shape
        # # print(seqlens) # real+5
        # # print(head_dim)
        # # exit()
        # return _triton_mixed_sparse_attention(q, k, v, seqlens, sm_scale = head_dim ** -0.5)
    ...
    def dense(q, k, v, vertical_size=None, slash_size=None):
        # 单头计算
        # print(q.transpose(1, 2).shape) # [1, seqlen, 1, 128]
        # print(k.transpose(1, 2).shape) # [1, seqlen, 1, 128]
        # print(v.transpose(1, 2).shape) # [1, seqlen, 1, 128]
        # print(q.shape[-2]) # 4091 16379
        # exit()
        # real_len = 20000 # 测试时使用64的倍数，后续调整
        # q = torch.randn(1, 1, real_len, 128).to(torch.bfloat16).cuda()
        # k = torch.randn(1, 1, real_len, 128).to(torch.bfloat16).cuda()
        # v = torch.randn(1, 1, real_len, 128).to(torch.bfloat16).cuda()
        # print(q)
        return flash_attn_func(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1).view(bsz, 1, q_len, self.head_dim)
        # return flash_attn_func3(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1).view(bsz, 1, q_len, self.head_dim) # my v3
    def block_sparse_kernel(q, k, v, vertical_size=None, slash_size=None):
        topk = 100
        return block_sparse_attention(q, k, v, topk)
    ...
    # print(f"head_id: {head_id}, pattern: {ty}, vertical_size: {vertical_size}, slash_size: {slash_size}")
    # exit()
...
def minference_forward():
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.select_mode:
            self.indecies = None
            find_context(self, query_states, key_states, self.topk)
        ...
        # if q_len != 1 and self.topk != q_len: # prefill... [GemFilter]需要改进，新一轮是否使用密集计算？ update this ********************************************************************
        if q_len != 1: # prefill...  # original ********************************************************************
            ...
            for head in range(query_states.size(1)):
                ...
                if self.layer_idx >= self.starting_layer and not self.is_search:
                    attn_output = self.gather_last_q_vertical_slash_topk_v4(q, k, v, head, self.layer_idx)
            ...
            output =  flash_attn_func(query_states.transpose(1, 2), key_states.transpose(1, 2), value_states.transpose(1,2), 0.0, softmax_scale=None, causal=q_len != 1).reshape(bsz, query_states.size(1), q_len, self.head_dim)
